...
export default async function TagListPage({
  ...
  const [list, counts] = await Promise.all([
    findPublishedPosts({ site: "faith", tag: name, page }),
  ...
  return (
    <ListPageView
      ...
      page={list}
      hrefFor={(target) =>
        target > 1 ? `/faith/tags/${tag}?page=${target}` : `/faith/tags/${tag}`
      }
      searchAction="/faith"
...
export async function generateMetadata({ params, searchParams }: PageProps<"/faith/tags/[tag]">) {
  const [{ tag }, query] = await Promise.all([params, searchParams]);
  const name = decodeURIComponent(tag);
  ...
  return {
    title: `#${name}`,
    alternates: siteAlternates("faith", `/faith/tags/${tag}${page}`),
  };
}
