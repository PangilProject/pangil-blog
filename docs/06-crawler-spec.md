...
- **무료 티어 정합**: Hobby 크론 = 하루 1회·시(hour) 내 임의 실행·UTC — watchdog는 하루 1회면 충분하고 분 단위 정밀도 불필요하므로 제약과 완전히 부합. **외부 서비스(healthchecks.io 등) 없이 기존 인프라(Vercel+DB+Slack)만으로 데드맨 스위치 완성** → 프리모템 #6(의존성 최소화) 준수.
- ⚠ **정정(M4 구현)**: 크론 개수는 넉넉하지 않다 — **Hobby는 프로젝트당 2개**(100개는 Pro 기준). 그래서 실제 등록은 두 개이고, watchdog는 아침 경로에 백업과 함께 실려 있다. `path`가 06 §5의 것과 다른 이유다.
  - `/api/cron/morning`  `0 2 * * *` (KST 11:00) — watchdog + DB 백업
  - `/api/cron/reminder` `0 12 * * *` (KST 21:00) — 미작성일 리마인드
  - `/api/crawler/watchdog`·`/api/cron/backup`은 남겨둔다 — 크론이 부르지 않고 손으로 확인할 때 쓴다.
...
| **검색엔진 자동 제출**(IndexNow) | 무침범 | 중상 | 저 | **중상** | 발행·삭제 시 트리거. SEO 인프라 MVP(00 §7-7)와 정합. **M6 정정**: "sitemap/RSS ping"과 "인덱싱 API 연동"은 지금 존재하지 않는 경로다(§10 #5). 실제 구현은 IndexNow 한 규격이고, 비용이 '중'에서 '저'로 내려간다 |
...
| # | 항목 | 상태 |
|---|------|------|
| 1 | 365qt 로그인 방식 · 페이지 URL·마크업 셀렉터 | ✅ M4. **폼 POST**(`USER_ID`/`Password` + ASP.NET antiforgery 토큰·쿠키). 캡차·JS 게이트 없어 헤드리스 불필요. 로그인 `/User/Login`, 본문 `/QT/TodayQT`. 셀렉터는 `scripts/crawler/parse.mts` 주석 |
| 2 | no-content(휴일/미게시) 감지 마커 | ✅ M4. 날짜 슬라이더 `li.qt-slider-box[data-lidate]`의 **`disable` 클래스**(+`title="주일은 오늘의큐티가 없습니다"`). 그 외의 빈 본문은 FAILED |
| 3 | 크롤 시각 | ✅ M4. **UTC 21:00 = KST 06:00** (`.github/workflows/crawl-qt.yml`) |
| 4 | DB 백업 저장 위치·주기 | ✅ M4. **Supabase Storage 비공개 버킷 `backups/db/{날짜}.json`, 하루 1장**. GitHub 커밋은 Actions에 DB 자격증명을 요구해 기각 |
| 5 | 검색엔진 제출 채널(구글 Indexing API·네이버 서치어드바이저) 범위 | ✅ M6. **IndexNow 하나**(네이버·Bing·Yandex 공동 규격, 네이버는 2023-07부터 지원). 구글은 자동화 경로가 없다 — sitemap ping은 2023년 폐지, Indexing API는 채용공고·라이브스트림 전용이라 블로그 글에 쓸 수 없어 **서치콘솔 sitemap 수동 등록**이 전부다. 네이버 "수집 요청 API"는 신디케이션에서 전환된 **제휴(승인) 경로**라 개인 블로그는 키를 받을 수 없다 |
| 6 | 미작성일 리마인드 시각·조건 | ✅ M4. **KST 21:00**, 그날 카드 타입(02 §3.1) 중 **발행되지 않은 것**이 하나라도 있으면 알림. 초안만 있는 것은 미완료 |

### 10.1 구현에서 추가로 확정한 것 (M4)

- **질문 6개의 실제 형태**: 365qt는 문단 5개로 준다 — 마지막 문단(5번)에 두 물음이 줄바꿈으로 들어 있고, 이를 `5-1`·`5-2`로 나눠 6질문이 된다. 02 §5.2 프리셋 라벨과 정확히 일치한다.
- **기도·은혜나눔 문단**이 질문과 같은 클래스(`p.font-b`)로 온다. 그래서 파서는 **아는 그룹 라벨(4종) 안에서만** 질문을 줍는다 — 그러지 않으면 7질문이 되어 매일 FAILED가 된다.
- **그룹 라벨 표기 차이**: 365qt "느낀점" ↔ 우리 스키마 "느낀 점". 공백을 지운 이름으로 맞춘다.
- **날짜 불일치**를 새 실패 모드로 추가했다: 페이지가 `runDate`와 다른 날짜를 보여주면 ParseError. 어제 큐티가 오늘 초안으로 들어오면 안 된다.
- 크롤러는 앱과 **같은 검증 함수**(`lib/crawler/qtContent`)를 쓴다. Actions 러너가 `@/` 별칭을 해상하도록 훅(`scripts/aliasHook.mts`)만 두고 번들러는 들이지 않았다.
...
10. (M6) 검색엔진 제출 = IndexNow 단일 규격. **도메인이 확정되기 전에는 보내지 않는다** — 임시 호스트로 색인을 만들면 실도메인 이전 시 301을 걸 수 없고, 그건 00 §6.3이 지적한 "SEO 제로 리셋"을 우리 손으로 한 번 더 만드는 것이다
