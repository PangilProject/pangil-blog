...
4. content safeParse 통과분 삽입, **status=PUBLISHED (전량 즉시 공개 — Phase 6-3 결정 (가))**
...
| 2 | 마이그레이션 파서 DOM 셀렉터·publishedAt 위치 | 구현 착수(실물 HTML 1개) |
| 3 | publishedAt 정확 날짜 확보 경로(RSS 크로스체크 채택 여부) | 7 or 구현 |
| 4 | Prisma 전용 DB 롤 분리(하드닝) 채택 여부 | Backlog |
| 5 | DraftSchema partial 유틸 구현(Zod v4) | 5 락파일 고정 시 |
