...
공개 페이지의 클라이언트 아일랜드는 **UI 4개**: **코드 복사 버튼, TOC 하이라이트, YouTube 임베드(lite 방식 — 클릭 전 썸네일만), 다크모드 토글**. 그 외는 정적 문서.

> **M6 정정.** 여기에 **계측 1개**(통계 비콘, 05 §4)가 더해진다. UI 아일랜드로 세지 않는 이유는 그리는 것이 없어서다 — 렌더 트리에 노드를 남기지 않고 조판에 관여하지 않으므로 "지면이 제각각 어긋난다"는 위험(프리모템 #12)과 무관하다. 대신 **한도의 취지는 그대로 적용한다**: 비콘이 하는 일은 읽고·쏘고·끝이며, 여기에 대시보드·A/B·개인화를 얹지 않는다. `AGENTS.md`가 "3개"로 적고 있던 것은 다크모드 토글이 빠진 오기이며 M6에서 이 문장으로 통일했다.
