 ...
 *
 * 지원: 제목(h2/h3) · 본문 · 코드 블록(언어) · 인용 · 목록(중첩) · 이미지 · 링크 · 구분선 · 표
 *
 * 표는 2026-09-12에 들어왔다(ADR-001 갱신 — "이미 지원하고 있었는데 누를 자리만 없었다").
 * 그 전까지 이 자리에 "미지원: 표"라고 적혀 있었고, 같은 파일이 이미 파이프 표를 읽고 있었다.
 */
